...
@endsection
@section('content')
    <form method="POST" action="{{ route('admin.branches.store') }}" class="space-y-4 bg-white/90 backdrop-blur p-5 md:p-6 rounded-xl shadow ring-1 ring-black/5">
        @csrf
        <div>
        ...
        <div>
            ...
            <textarea id="address" name="address" class="w-full border rounded-lg p-2 h-20" required>{{ old('address') }}</textarea>
            @error('address')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        <div>
            ...
            <input type="email" id="email" name="email" value="{{ old('email') }}" class="w-full border rounded-lg p-2" />
            @error('email')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        <div>
            ...
            <input id="whatsapp" name="whatsapp" value="{{ old('whatsapp') }}" placeholder="+234 9160006956" class="w-full border rounded-lg p-2" />
            @error('whatsapp')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        <div>
            ...
            <input id="phone" name="phone" value="{{ old('phone') }}" placeholder="+234 2013306189" class="w-full border rounded-lg p-2" />
            @error('phone')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="flex gap-3">
            <button type="submit" class="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90">Create Branch</button>
