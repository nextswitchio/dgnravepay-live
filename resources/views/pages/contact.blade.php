        ...
        <section class="bg-accent-black relative -z-[0]">
            <div class="custom-container mx-auto  px-5 md:px-10 py-20 md:py-28">
                <div class="text-center">
                    ...
                    <div class="mt-5 inline-flex bg-white rounded-full p-2 flex-wrap gap-1">
                        @forelse ($states as $index =>$state)
                            <button 
                                class="state-tab px-3 py-2 rounded-full flex gap-2 font-semibold {{ $index === 0 ? 'border border-primary' : '' }}" 
                        ...
                        @empty
                            <p class="px-3 py-2 text-gray-600">No state locations available.</p>
                        @endforelse
                    </div>
                </div>
                <div class="md:grid gap-10 md:grid-cols-7 mt-20">
                    <div class="md:col-span-3 text-white font-medium space-y-5">
                        @foreach ($states as $index => $state)
                            <div class="state-branches" data-state-id="{{ $state->id }}" style="{{ $index === 0 ? '' : 'display: none;' }}">
                                @forelse ($state->branches as $branch)
                                    <div class="bg-primary rounded-xl p-5 divide-y divide-white/30 mb-5">
                                        <p class="flex items-center gap-2 pb-5">
                                            <img src="{{ Vite::asset('resources/images/round-nigerian-flag-of-nigeria-vector.jpg') }}"
                                                alt="" class="w-10 rounded-full">
                                            <span class="text-lg md:text-xl font-semibold">{{ $branch->name }}</span>
                                        </p>
                                        <ul class="pt-5 space-y-3">
                                            ...
                                            @if ($branch->whatsapp)
                                                <li class="flex gap-3 items-center">
                                                    ...
                                                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $branch->whatsapp) }}" target="_blank">
                                                        <p>{{ $branch->whatsapp }}</p>
                                                    </a>
                                                </li>
                                            @endif
                                ...
                                @empty
                                    <div class="bg-primary rounded-xl p-5 text-center">
                                        <p>No branches available in {{ $state->name }}.</p>
                                    </div>
                                @endforelse
                            </div>
                        @endforeach
            ...
            <script>
                document.addEventListener('DOMContentLoaded', () => {
                    ...
                    const stateBranches = document.querySelectorAll('.state-branches');
                    
                    stateTabs.forEach(tab => {
                        tab.addEventListener('click', () => {
                            const stateId = tab.dataset.stateId;
                            
                            // Update active tab styling
                            stateTabs.forEach(t => {
                                t.classList.remove('border', 'border-primary');
                            });
                            tab.classList.add('border', 'border-primary');
                            
                            // Show/hide branches by state
                            stateBranches.forEach(branches => {
                                if (branches.dataset.stateId === stateId) {
